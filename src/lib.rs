    ...
    fn wiggle_cursor() {
        ...
        for _ in 0..1000 {
            ...
            match rng.gen_range(0..4) {
                0 => {
                    buf_state.move_cursor_down(amount);
                    println!("moved down by {amount}");
                }
                1 => {
                    buf_state.move_cursor_up(amount);
                    println!("moved up by {amount}");
                }
                2 => {
                    buf_state.move_cursor_left(amount);
                    println!("moved left by {amount}");
                }
                _ => {
                    buf_state.move_cursor_right(amount);
                    println!("moved right by {amount}");
                }
            }

            let line = &buf_state.buf[buf_state.scrolled + buf_state.c_row];

            assert!(buf_state.c_row < buf_state.c_row_max);
            assert!(buf_state.scrolled + buf_state.c_row < buf_state.buf.len());
            if line.len() > 0 {
                assert!(buf_state.c_col < line.len());
            } else {
                assert!(buf_state.c_col == 0);
            }
        }
    }
}
...
impl BufState {
    fn adjust_col(&mut self) {
        let line = &self.buf[self.scrolled + self.c_row];
        let mut sub = 0;
        ...
        self.c_col = cmp::min(self.c_col, line.len() - sub);
    }
    ...
    fn redo(&mut self) {
        ...
        let UndoBufferEntry(_, l1n, l2o, l2n, _, cpn) = &self.un_buf[self.un_buf_i];
        ...
        if target_line <= last_visible_line {
            ...
            if curr_line < last_visible_line {
                ...
                if last_visible_line < self.buf.len() {
                    ...
                    self.scrolled = cmp::min(self.scrolled, self.buf.len() - self.c_row - 1)
                }
            } else {
                // Just scroll
    ...
    fn move_cursor_right(&mut self, amount: usize) {
        ...
        self.c_col = cmp::min(self.c_col + amount, line.len() - sub);
    }
...
pub fn run(filename: &str) -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        if let Event::Key(KeyEvent { code, modifiers: m }) = read()? {
            match code {
                KeyCode::Esc => break,
                KeyCode::Up => (buf_state.move_cursor_up(1)),
                KeyCode::Down => (buf_state.move_cursor_down(1)),
                KeyCode::Left => (buf_state.move_cursor_left(1)),
                KeyCode::Right => (buf_state.move_cursor_right(1)),
                KeyCode::PageUp => (buf_state.move_cursor_up(10)),
                KeyCode::PageDown => (buf_state.move_cursor_down(10)),
                KeyCode::Backspace => (buf_state.backspace()),
                KeyCode::Enter => (buf_state.enter()),
                KeyCode::Char(c) => {
